@include('contaminant.partials._buttons', ['class' => 'buttons-top'])
            ...
            <div class="tab-content">
                @foreach (config('app.locales') as $key => $locale)
                    <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_{!! $locale !!}">
                        @include('views.contaminant.tabs.locale', ['errors' => $errors, 'model' => $model , 'locale' => $locale])
                    </div>
                @endforeach

                <div class="tab-pane" id="general">
                    @include('contaminant.tabs.general')
                </div>
            </div>
        </div>
...
@include('contaminant.partials._buttons')